...
public class AVLTree<K extends Comparable<K>, V> {
    ...
    // 向以node为根的二分搜索树中插入元素(key, value)，递归回溯算法
    // 返回插入新节点后二分搜索树的根
    private Node add(Node node, K key, V value) {
        ...
        //平衡维护
        //LL
        if (balanceFactor > 1 && getBalancedFactor(node.left) >= 0) //插入的节点在左侧的左侧
            return rightRotate(node);

        //RR
        if (balanceFactor < -1 && getBalancedFactor(node.right) <= 0) //插入的节点在右侧的右侧
...
//        }
        ...
        //平衡维护
        //LL
        if (balanceFactor > 1 && getBalancedFactor(retNode.left) >= 0) //插入的节点在左侧的左侧
            return rightRotate(retNode);

        //RR
        if (balanceFactor < -1 && getBalancedFactor(retNode.right) <= 0) //插入的节点在右侧的右侧
